...
if __name__ == '__main__':
    ...
    fig1, ax1 = pyplot.subplots(3)
    fig1.suptitle('Pontos Igualmente Espaçados')

    ax1[0].set_ylim(-0.5, 1.5)
    ax1[1].set_ylim(-0.5, 1.5)
    ax1[2].set_ylim(-0.5, 1.5)
    ...
    for j, n in enumerate(iterations):
        ...
        ax1[j].plot(X, [lagrange(x_eq, y_eq, x) for x in X], 'r-')
        ax1[j].scatter(x_eq, y_eq, c='blue', s=10)
